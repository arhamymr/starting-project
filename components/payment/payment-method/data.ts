export const data = [
  {
    title: "1. Transfer Bank",
    items: [
      {
        logo: "/assets/payment/bri.png",
        label: "Bank Rakyat Indonesia (BRI)",
      },
      {
        logo: "/assets/payment/bni.png",
        label: "Bank Negara Indonesia (BNI)",
      },
      {
        logo: "/assets/payment/mandiri.png",
        label: "Bank Mandiri",
      },
      {
        logo: "/assets/payment/bca.png",
        label: "Bank BCA",
      },
    ],
  },
  // {
  //   title: "2. Virtual Akun",
  //   items: [
  //     {
  //       logo: "/assets/payment/bri.png",
  //       label: "Bank Rakyat Indonesia (BRI)",
  //     },
  //     {
  //       logo: "/assets/payment/bni.png",
  //       label: "Bank Negara Indonesia (BNI)",
  //     },
  //     {
  //       logo: "/assets/payment/mandiri.png",
  //       label: "Bank Mandiri",
  //     },
  //     {
  //       logo: "/assets/payment/bca.png",
  //       label: "Bank BCA",
  //     },
  //   ],
  // },
  // {
  //   title: "3. QRIS",
  //   items: [
  //     {
  //       logo: "/assets/payment/mandiri.png",
  //       label: "mandir",
  //     },
  //     {
  //       logo: "/assets/payment/jenius.png",
  //       label: "Jenius",
  //     },
  //     {
  //       logo: "/assets/payment/bsi.png",
  //       label: "BSI",
  //     },
  //     {
  //       logo: "/assets/payment/jago.png",
  //       label: "Jago",
  //     },
  //   ],
  // },
  // {
  //   title: "4. Tunai di Gerai Retail",
  //   items: [
  //     {
  //       logo: "/assets/payment/alfamart.png",
  //       label: "Alfamart",
  //     },
  //     {
  //       logo: "/assets/payment/alfamidi.png",
  //       label: "Alfamidi",
  //     },
  //     {
  //       logo: "/assets/payment/mandiri.png",
  //       label: "Indomaret",
  //     },
  //   ],
  // },
  // {
  //   title: "5. eWallet",
  //   items: [
  //     {
  //       logo: "/assets/payment/ovo.png",
  //       label: "OVO",
  //     },
  //     {
  //       logo: "/assets/payment/gopay.png",
  //       label: "GoPay",
  //     },
  //     {
  //       logo: "/assets/payment/shopepay.png",
  //       label: "Shopee Pay",
  //     },
  //     {
  //       logo: "/assets/payment/dana.png",
  //       label: "Dana",
  //     },
  //     {
  //       logo: "/assets/payment/linkaja.png",
  //       label: "LinkAja",
  //     },
  //   ],
  // },
];
